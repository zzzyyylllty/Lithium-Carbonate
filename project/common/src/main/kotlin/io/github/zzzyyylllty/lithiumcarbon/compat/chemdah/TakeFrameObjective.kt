...
object TakeFrameEventObjective: ObjectiveCountableI<FrameCrateClaimEvent>() {

    override val name = "lithium frame take"
    override val event = FrameCrateClaimEvent::class.java

    init {
        handler { it.player }

        addSimpleCondition("id") { data, e ->
            data.toString() == e.frameCrateConfig.id
        }

        addSimpleCondition("glow") { data, e ->
            data.toBoolean() == e.frameCrateConfig.glow
        }
...
fun registerTakeFrameEventObjective(){
    if (Bukkit.getPluginManager().isPluginEnabled("Chemdah")) TakeFrameEventObjective.register()
}
