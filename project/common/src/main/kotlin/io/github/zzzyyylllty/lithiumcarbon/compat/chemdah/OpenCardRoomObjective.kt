...
object OpenCardRoomObjective: ObjectiveCountableI<CardRoomOpenEvent>() {

    override val name = "lithium cardroom open"
    override val event = CardRoomOpenEvent::class.java

    init {
        handler { it.player }

        addSimpleCondition("id") { data, e ->
            data.toString() == e.instance.configId
        }
...
fun registerOpenCardRoomObjective(){
    if (Bukkit.getPluginManager().isPluginEnabled("Chemdah")) OpenCardRoomObjective.register()
}
